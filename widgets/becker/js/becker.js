...
// this code can be placed directly in becker.html
vis.binds.becker = {
    version: "0.5.0",
    showVersion: function () {
        if (vis.binds.becker.version) {
            console.log('Version becker: ' + vis.binds.becker.version);
            vis.binds.becker.version = null;
        }
    },
	createWidget: function (widgetID, view, data, style) {
        ...
        if (!$div.length) {
            return setTimeout(function () {
                vis.binds.becker.createWidget(widgetID, view, data, style);
            }, 100);
        }
        ...
        if (data.oid) {
            vis.states.bind(data.oid + '.val', function (e, newVal, oldVal) {
                $div.find('.becker-value').html(newVal);
            });
        }
    }
};
	
vis.binds.becker.showVersion();
